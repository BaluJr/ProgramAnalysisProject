...
INPUT_DATA_FILE = os.environ.get("INPUT_DATA_FILE", "../data/samples/Histories.txt")
...
def sgd_callback(model, num_examples_seen):
  ...
  #save_model_parameters_theano(model,"../data/rnn-theano-%d-%d-%s.npz" % (model.hidden_dim, model.word_dim, time))
  save_model_parameters_theano(model, MODEL_OUTPUT_FILE)
  print("\n")
  sys.stdout.flush()

for epoch in range(NEPOCH):
  train_with_sgd(model, x_train, y_train, learning_rate=LEARNING_RATE, nepoch=3, decay=0.9, 
